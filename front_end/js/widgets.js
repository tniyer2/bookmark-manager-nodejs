...
function prependBEMBlock(classObject, block) {
    if (block)
    {
...
function styleOnFocus(elm, cl, options) {
    return new AwesomeFocus(() => {
        addClass(elm, cl);
    }, () => {
        removeClass(elm, cl);
    }, options);
        ...
        load(info) {
            return new Promise((resolve, reject) => {
                let done = (elm, maxHeight) => {
                    ...
                    let finalMaxHeight = minIfNumber(maxHeight, this._options.maxHeight);
                    if (finalMaxHeight)
                    {
...
const AwesomeAlerter = function(){
    ...
    return class {
        ...
        _updateCss(adding)
        {
            ...
            {
                addClass(nodes[0], cl_singleLi);
            }
            else
            {
                if (adding === true && len > 1)
                {
                    removeClass(nodes[len-2], cl_lastLi);
                    removeClass(nodes[len-2], cl_singleLi);
                }
                addClass(nodes[0], cl_firstLi);
                addClass(nodes[len-1], cl_lastLi);
            }
        }
    };
}();
...
const ListManager = function(){
    ...
    return class {
        constructor(el_parent, options)
        {
            ...
            this._queue = new DOMQueue(this._el_list);
        }
        ...
        async addSource(srcUrl, sourceOptions)
        {
            ...
            if (sourceOptions.type === "image")
            {
                try
                {
                    sourceMeta = await this._testImageDimensions(srcUrl);
            ...
            else if (sourceOptions.type === "video")
            {
                try
                {
                    sourceMeta = await this._testVideoDimensions(srcUrl);
            ...
            {
                console.warn(
                    "sourceOptions.type is not 'image' or 'video':",
                    sourceOptions.type
                );
            }
        ...
        {
            let svg = createSVG("#icon-download");
            svg.classList.add(CLASSES.svg);
...
const AutoComplete = function(){
    ...
    return class {
        ...
        _attachEvents()
        {
            ...
            onKeyDown(this._el_input, "ArrowUp", (evt) => {
                evt.preventDefault();
                this._scroll(false);
            });
            onKeyDown(this._el_input, "ArrowDown", (evt) => {
                evt.preventDefault();
                this._scroll(true);
            });
            onKeyDown(this._el_input, "Enter", (evt) => {
...
export {
    DOMQueue, Toggle, ContentCreator, styleOnFocus,
    RadioManager, ListManager, AwesomeAlerter, AutoComplete
};
